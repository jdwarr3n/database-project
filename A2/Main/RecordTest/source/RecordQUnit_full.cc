...
#define FALLTHROUGH_INTENDED QUNIT_IS_TRUE(result)
...
template <typename Fn,
		  typename Rep	 = std::chrono::minutes::rep,
		  typename Period = std::chrono::minutes::period>
bool test(Fn &&testFn, const std::chrono::duration<Rep, Period> &duration = std::chrono::minutes(1))
{
	return testFn();
}
