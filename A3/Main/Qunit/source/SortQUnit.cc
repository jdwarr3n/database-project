...
int main() {
...

  {
    ...
    mySchema->appendAtt(
        make_pair("address", make_shared<MyDB_StringAttType>()));
    ...
    mySchema->appendAtt(
        make_pair("acctbal", make_shared<MyDB_DoubleAttType>()));
    mySchema->appendAtt(
        make_pair("comment", make_shared<MyDB_StringAttType>()));
    ...
    MyDB_BufferManagerPtr myMgr =
        make_shared<MyDB_BufferManager>(131072, 128, "tempFile");
...

  {
    ...
    MyDB_PageReaderWriterPtr sorted =
        supplierTable[36].sort(myComp, temp, temp2);
...

  {
    ...
    // use the schema to create a table
    MyDB_TablePtr outTable =
        make_shared<MyDB_Table>("supplierSorted", "supplierSorted.bin",
                                allTables["supplier"]->getSchema());
